...
f_exp1 <- function(seed){
  ...
  for (i in 1:n){
    if(X[i,1]>0&X[i,2]>0){
      M[i] = rnorm(1,0,0.01) +1*X[i,3] +1*X[i,4] +0.5*X[i,1]+0.5*X[i,2] + W[i]
    ...
    if(X[i,1]>0&X[i,2]>0){
      Y[i] = 1 + 1*M[i] + rnorm(1,0,0.01)+ 0.5*X[i,3] +0.5*X[i,4]+0
      #Y_CT[i] = Y[i] - 1*M[i] + M_CT[i] - (1*X[i,1]*W[i] +1*X[i,2]*W[i]) 
    }else{
  ...
  data_save_path = paste0("/Users/xli36/Downloads/R_packages/myproject/expe5_new/seed_",seed,".csv")
  ...
  save_path = paste0("/Users/xli36/Downloads/R_packages/myproject/expe5_new/seed_",seed,".rds")
...
my_new_folder = "/Users/xli36/Downloads/R_packages/myproject/expe5_new"
