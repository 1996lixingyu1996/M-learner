...
f_exp1 <- function(seed){
  ...
  for (i in 1:n){
    ...
    if(X[i,1]>0&X[i,2]>0){
      Y[i] = 1 + 1*M[i] + rnorm(1,0,0.01)+ 0.5*X[i,3] +0.5*X[i,4]+1*X[i,1]*W[i] +1*X[i,2]*W[i]
      p <- 1 / (1 + exp(Y[i]))
      Y[i] = p
  ...
  data_save_path = paste0("/Users/xli36/Downloads/R_packages/myproject/expe7_new/seed_",seed,".csv")
  ...
  save_path = paste0("/Users/xli36/Downloads/R_packages/myproject/expe7_new/seed_",seed,".rds")
...
my_new_folder = "/Users/xli36/Downloads/R_packages/myproject/expe7_new"
