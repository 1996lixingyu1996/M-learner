...
f_exp1 <- function(seed){
  ...
  for (i in 1:n){
    if(X[i,1]>0&X[i,2]>0){
      Y[i] = 1 + W[i] + rnorm(1,0,0.01)+1*X[i,3] + 1*X[i,4] #1*X[i,3] + 1*X[i,4] + 1*X[i,1]*W[i] + 1*X[i,2]*W[i] + rnorm(1,0,0.01)+0.5*X[i,1]+0.5*X[i,2]
      ...
      Y[i] = 1 + W[i] + rnorm(1,0,0.01)+1*X[i,3] + 1*X[i,4]#1*X[i,3] + 1*X[i,4] + 1*X[i,1]*W[i] + 1*X[i,2]*W[i] + rnorm(1,0,0.01)+0.5*X[i,1]+0.5*X[i,2]#1 + 1*X[i,3] + 1*X[i,4] + rnorm(1,0,0.01)+0.5*X[i,1]+0.5*X[i,2]
      #p <- 1 / (1 + exp(Y[i]))
      #Y[i] = p
  ...
  data_save_path = paste0("/Users/xli36/Downloads/R_packages/myproject/expe5_nomediator/seed_",seed,".csv")
  rio::export(data, data_save_path)
  save_path = paste0("/Users/xli36/Downloads/R_packages/myproject/expe5_nomediator/seed_",seed,".rds")
  result[[1]] = MSE
  result[[2]] = MAE
  result[[3]] = data_flag
  result[[4]] = pval_flag
...
my_new_folder = "/Users/xli36/Downloads/R_packages/myproject/expe5_nomediator"
