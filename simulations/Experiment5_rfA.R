...
f_exp1 <- function(seed){
  ...
  for (i in 1:n){
    ...
    if(X[i,1]>0&X[i,2]>0){
      Y[i] = 1 + 0*M[i] + rnorm(1,0,0.01)+ 0.5*X[i,3] +0.5*X[i,4]+1*X[i,1]*W[i] +1*X[i,2]*W[i]
      #Y_CT[i] = Y[i] - 1*M[i] + M_CT[i] - (1*X[i,1]*W[i] +1*X[i,2]*W[i]) 
    }else{
      Y[i] = 1 + 0*M[i] + rnorm(1,0,0.01)+ 0.5*X[i,3] +0.5*X[i,4]
...
#data = rio::import(paste0("path/seed_",1,".rds"))
library(Rtsne)
library(rio)
library(snowfall)
library(parallel)
